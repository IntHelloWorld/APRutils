...
public class App {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		if (s_CommandLineValues.projectDir != null && s_CommandLineValues.classPath != null
			...
			MethodExtractTask featureExtractTask = new MethodExtractTask(s_CommandLineValues, line,
					new ProgressBar(1));
	...
	private static void solveAll(File csvFile) {
		...
		// LinkedList<MethodExtractTask> tasks = new LinkedList<>();
		try {
			...
			while ((line = reader.readNext()) != null) {
				...
				MethodExtractTask task = new MethodExtractTask(s_CommandLineValues, currentLine, progressBar);
				task.processFile();
				// tasks.add(task);
				lastLine = new Line(line);
			}
